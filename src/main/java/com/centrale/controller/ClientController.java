...
private void cancelOrder(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
        ...
        if (!orderOptional.isPresent() || !orderOptional.get().getClient().getUser().getId().equals(currentUser.getId())) {
            LOGGER.warning("Attempt to cancel non-existent or unauthorized order");
            response.sendRedirect(request.getContextPath() + "/client/orders?error=Order+not+found+or+unauthorized");
            return;
        }
        ...
        if (order.getStatus() == OrderStatus.SHIPPED || order.getStatus() == OrderStatus.DELIVERED || order.getStatus() == OrderStatus.CANCELLED) {
            ...
            response.sendRedirect(request.getContextPath() + "/client/orders?error=Cannot+cancel+order+with+status+" + order.getStatus());
            return;
        }
        ...
        response.sendRedirect(request.getContextPath() + "/client/orders?error=Error+cancelling+order:+" + e.getMessage());
    }
}
...
private void deleteOrder(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
        ...
        if (order.getStatus() != OrderStatus.CANCELLED) {
            ...
            response.sendRedirect(request.getContextPath() + "/client/orders?error=Only+cancelled+orders+can+be+deleted");
            return;
        }
        ...
        response.sendRedirect(request.getContextPath() + "/client/orders?error=Error+deleting+order:+" + e.getMessage());
    }
}
